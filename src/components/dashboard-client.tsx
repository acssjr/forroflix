...
import { 
  ...
  Sun, Moon, Menu, Video, ChevronLeft, Camera, X, Loader2
} from 'lucide-react';
...
export function DashboardClient({
  ...
  const [currentUserFullName, setCurrentUserFullName] = useState(user.full_name || '');
  const [currentUserAvatarUrl, setCurrentUserAvatarUrl] = useState(user.avatar_url || '');
  const [editProfileFullName, setEditProfileFullName] = useState(user.full_name || '');
  const [editProfileAvatarUrl, setEditProfileAvatarUrl] = useState(user.avatar_url || '');
  ...
  const handleSaveProfile = async (e: React.FormEvent) => {
    ...
    try {
      const res = await updateProfileAction(editProfileFullName, editProfileAvatarUrl);
      if (res.error) throw new Error(res.error);
      
      setCurrentUserFullName(editProfileFullName);
      setCurrentUserAvatarUrl(editProfileAvatarUrl);
      ...
      console.error(err);
      setProfileError(err.message || 'Falha ao salvar perfil.');
    } finally {
        ...
        <main className="flex-grow px-4 sm:px-6 md:px-8 py-8 space-y-8 w-full">
          ...
          <div className="text-left space-y-1">
            <h1 className="text-2xl font-black tracking-tight text-foreground leading-tight">
              Olá, {(currentUserFullName ? currentUserFullName.trim().split(/\s+/)[0] : '') || user.username || user.email.split('@')[0] || 'Aluno'}!
            </h1>
        ...
        <aside className="w-full lg:w-90 bg-sidebar text-sidebar-foreground border-t lg:border-t-0 lg:border-l border-sidebar-border p-6 space-y-8 shrink-0">
          ...
          <div className="flex items-center justify-between">
            <button 
              ...
              className="flex items-center gap-4 text-left group hover:opacity-95 transition-all focus:outline-none cursor-pointer w-full"
              title="Editar Perfil"
            >
              <div className="w-14 h-14 rounded-full bg-primary/10 border border-primary/20 flex items-center justify-center font-black text-primary text-lg shadow-md relative overflow-hidden shrink-0 group-hover:border-primary/45 transition-colors">
                {currentUserAvatarUrl ? (
                  ...
                  (currentUserFullName || user.username || user.email).substring(0, 2).toUpperCase()
                )}
              ...
              <div className="text-left min-w-0">
                <span className="text-sm font-bold text-sidebar-foreground/90 block group-hover:text-primary transition-colors leading-tight truncate">
                  {(currentUserFullName ? currentUserFullName.trim().split(/\s+/)[0] : '') || user.username || user.email.split('@')[0] || 'Aluno'}
                </span>
          ...
          <div className="bg-card text-card-foreground border border-border w-full max-w-md rounded-3xl overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleSaveProfile} className="p-6 space-y-5 text-left">
              ...
              <div className="flex flex-col items-center justify-center gap-3">
                <div className="w-20 h-20 rounded-full bg-primary/10 border border-primary/20 flex items-center justify-center font-black text-primary text-2xl shadow-md relative overflow-hidden">
                  {editProfileAvatarUrl ? (
                    ...
                    (editProfileFullName || user.username || user.email).substring(0, 2).toUpperCase()
                  )}
                </div>

                <div className="flex gap-2">
                  <label className="text-[10px] bg-secondary hover:bg-secondary/80 border border-border text-foreground hover:text-primary font-bold px-3 py-1.5 rounded-lg cursor-pointer transition-all">
                    Escolher Foto
                    <input 
                      type="file" 
                      accept="image/*" 
                      ...
                      onChange={async (e) => {
                        ...
                        if (file) {
                          if (file.size > 5 * 1024 * 1024) {
                            setProfileError('A imagem selecionada é muito grande. Escolha uma imagem de até 5MB.');
                            return;
                          }
                          setProfileError(null);
                          setProfileLoading(true);
                          try {
                            ...
                            setEditProfileAvatarUrl(compressedBase64);
                          } catch (err: any) {
                            console.error('Erro ao processar imagem:', err);
                            setProfileError('Erro ao processar a imagem. Tente outro arquivo.');
                          } finally {
                            setProfileLoading(false);
                          }
                        }
                  ...
                  {editProfileAvatarUrl && (
                    <button 
                      type="button"
                      onClick={() => setEditProfileAvatarUrl('')}
                      className="text-[10px] bg-destructive/10 border border-destructive/20 text-destructive hover:bg-destructive hover:text-white font-bold px-3 py-1.5 rounded-lg cursor-pointer transition-all"
                    >
                      Remover Foto
                    </button>
                  )}
              ...
              <div>
                ...
                <input 
                  ...
                  placeholder="Ex: Antônio Santos"
                  className="w-full bg-background text-foreground placeholder-muted-foreground/60 border border-border rounded-xl px-4 py-2.5 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary text-xs transition-colors"
                />
              </div>

              {/* Erro */}
              {profileError && (
                <div className="p-3 bg-destructive/10 border border-destructive/20 rounded-xl text-[10px] text-destructive">
                  {profileError}
                </div>
              )}
              ...
              <div className="flex justify-end gap-3 pt-2">
                <Button 
                  ...
                  variant="outline" 
                  onClick={() => setShowProfileModal(false)}
                  className="border-border hover:bg-secondary text-foreground text-xs cursor-pointer"
                >
                  Cancelar
                </Button>
                <Button 
                  ...
                  className="bg-primary hover:bg-primary/90 text-primary-foreground font-bold px-5 text-xs cursor-pointer"
                >
                  {profileLoading ? (
                    <>
